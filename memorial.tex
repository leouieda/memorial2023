...
Quando ele e o Mario me convidaram para coorientá-lo, fiquei feliz em aceitar
para continuar trabalhando com ele.
Inicialmente, a proposta era para eu ser seu orientador principal.
Mas como essa seria minha primeira orientação e além disso seria inteiramente
de forma remota, achei mais prudente começar com coorientador.
O Santiago atuou nas linha de pesquisa em modelagem com tesseroides
(seção~\ref{sec_modelagemdireta} e camada equivalente (seção~\ref{sec_eql})
durante seu doutorado.
Seu envolvimento no Fatiando a Terra aumentou gradativamente com o tempo.
Hoje em dia, ele é ocupa uma posição de liderança no projeto e é o principal
desenvolvedor dos softwares
\href{https://www.fatiando.org/harmonica}{Harmonica} e
...
minha carreira.
Santiago é um pesquisador e programador brilhante.
...
\begin{summarybox}[frametitle=\faInfoCircle{}\quad Resumo da linha de pesquisa]
  \begin{fa-ul}
    ...
    \faBullseye & Objetivos principais:
      Construir modelos compactos capazes de recuperar a forma 3D de corpos geológicos a partir de dados geofísicos.
      Superar as limitações computacionais da inversão 3D em métodos potenciais devido ao elevado número de dados e elementos no modelo.
      Providenciar maior nível de controle ao intérprete para incorporar conhecimentos sobre a geologia do alvo.
  \end{fa-ul}
\end{summarybox}

Linha de pesquisa iniciada durante meu mestrado no Observatório Nacional
(seção~\ref{sec_on})
Seed e trabalhos do Dio.
Inversão do Guangdong.

Minhas principais contribuições nesse tema foram:
...
\begin{itemize}
  ...
  \item ALGO SOBRE O MÉTODO DIFERENTE
  \item Desenvolvimento de um método eficiente para inversão 3D em uma
    aproximação esférica \citep{Zhao2019}
    (colaboração com pesquisadores da Central South University, China, e GFZ
    Potsdam, Alemanha).
\end{itemize}

Os principais impactos dessa pesquisa foram:
